<?php declare(strict_types=1);

namespace App\Modules\Designation;

use App\Http\Controllers\Controller;
use App\Exceptions\ValidationException;
use App\Modules\Designation\DesignationService as Service;
use App\Modules\Designation\DesignationValidation as Validation;
use Illuminate\Support\Facades\Validator;
use Illuminate\Http\Request;

final class DesignationController extends Controller 
{
    protected $service;

    public function __construct(Service $service)
    {
        $this->service = $service;
    }

    public function index()
    {
        guard('designation-view');
        return view('designation.index')
            ->with('title', __('message.designation_list'));
    }

    public function datalist()
    {
        guard('designation-view');
        $response = $this->service->getDataTableList();
        return $this->success($response);
    }

    public function create()
    {
        guard('designation-create');
        return view('designation.edit')
            ->with('title', __('message.new_designation'));
    }

    public function store(Request $request) 
    {
        guard('designation-create');
        try 
        {
            $validator = Validator::make(
                $request->all(), 
                Validation::getRules()
            );

            if ($validator->fails()) 
                return $this->error($validator->errors());

            $response = $this->service->create($validator->validated());
            return $this->created($response);
        }
        catch (Throwable $exception) 
        {
            return $this->handler($error);
        }
    }

    public function edit($id)
    {
        guard('designation-update');
        $row = $this->service->findById($id);
        return view('designation.edit', compact('id', 'row'))
            ->with('title', __('message.edit_designation'));
    }

    public function show($id)
    {
        guard('designation-update');
        try 
        {
            $response = $this->service->findById($id);
            return $this->success($response);
        }
        catch (Throwable $exception) 
        {
            return $this->handler($error);
        }
    }

    public function update(Request $request, $id) 
    {
        guard('designation-update');
        try 
        {
            $validator = Validator::make(
                $request->all(), 
                Validation::getRules((int) $id)
            );

            if ($validator->fails()) 
                return $this->error($validator->errors());

            $this->service->update($validator->validated(), $id);
            return $this->updated();
        }
        catch (Throwable $exception) 
        {
            return $this->handler($error);
        }
    }

    public function destroy($id)
    {
        guard('designation-delete');
        try 
        {
            $this->service->destroy($id);
            return $this->deleted();
        }
        catch (Throwable $exception) 
        {
            return $this->handler($exception);
        }
    }
}